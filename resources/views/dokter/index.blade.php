...
@section('title', 'Dokter RSUD Madani Palu')
@section('meta_description', 'Tim dokter spesialis profesional RSUD Madani Palu yang siap melayani kebutuhan kesehatan Anda.')
    ...
    <section class="section-padding">
        <div class="container">
                    ...
                    <div class="row g-3 align-items-end">
                        <div class="col-lg-5">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="search" class="form-control border-start-0 ps-0" placeholder="Nama dokter..." value="{{ request('search') }}" form="filterForm">
                            </div>
                        </div>
                        <div class="col-lg-4">
                            ...
                            <select name="spesialis" class="form-select" form="filterForm">
                                ...
                                @foreach($spesialis as $item)
                                    <option value="{{ $item->id }}" {{ request('spesialis') == $item->id ? 'selected' : '' }}>
                                        {{ $item->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-3">
                            <form action="{{ route('dokter.index') }}" method="GET" id="filterForm">
                                <input type="hidden" name="search" value="{{ request('search') }}">
                                <input type="hidden" name="spesialis" value="{{ request('spesialis') }}">
                            </form>
                            <div class="d-grid gap-2">
                                ...
                                @if(request('search') || request('spesialis'))
                                    <a href="{{ route('dokter.index') }}" class="btn btn-outline-secondary rounded-pill py-2 small">
            ...
            <div class="d-flex align-items-center justify-content-between mb-4" data-aos="fade-up">
                <div>
                    <span class="fw-semibold text-primary">{{ $dokter->total() }}</span>
                    <span class="text-muted">Dokter ditemukan</span>
                    @if(request('search'))
                        <span class="text-muted ms-2">untuk "<strong>{{ request('search') }}</strong>"</span>
                    @endif
                </div>
                @if($spesialis->count() > 0)
                    <div class="d-none d-md-flex gap-1 flex-wrap">
                        <a href="{{ route('dokter.index') }}" class="btn btn-sm {{ !request('spesialis') ? 'btn-secondary' : 'btn-outline-secondary' }} rounded-pill">Semua</a>
                        @foreach($spesialis as $item)
                            <a href="{{ route('dokter.index', ['spesialis' => $item->id, 'search' => request('search')]) }}"
                               class="btn btn-sm {{ request('spesialis') == $item->id ? 'btn-secondary' : 'btn-outline-secondary' }} rounded-pill">
                                {{ $item->nama }}
                            </a>
                        @endforeach
                    </div>
            ...
            <div class="row g-4">
                        ...
                        <div class="card card-dokter h-100 border-0 shadow-sm">
                            <div class="card-img-wrapper">
                                ...
                                <div class="card-img-overlay d-flex align-items-end p-3">
                                    <span class="badge bg-secondary rounded-pill">{{ $item->spesialis->nama ?? 'Umum' }}</span>
                                </div>
                                <div class="dokter-social-hover">
                                    @if($item->telepon)
                                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $item->telepon) }}" target="_blank" title="WhatsApp">
                                            <i class="bi bi-whatsapp"></i>
                                        </a>
                                    @endif
                                    <a href="{{ route('dokter.detail', $item->id) }}" title="Lihat Detail">
                            ...
                            <div class="card-body text-center">
                                ...
                                <p class="card-text text-muted small mb-3">{{ $item->spesialis->nama ?? 'Dokter Umum' }}</p>
                                <div class="d-flex align-items-center justify-content-center gap-2 mb-3">
                                    <small class="text-muted"><i class="bi bi-clock me-1"></i>{{ $item->jadwal->where('status', 'aktif')->count() }} Jadwal</small>
                                </div>
                                <a href="{{ route('dokter.detail', $item->id) }}" class="btn btn-outline-primary btn-sm rounded-pill px-3">
                ...
                @empty
                    <div class="col-12 text-center py-5" data-aos="fade-up">
                        ...
                        <a href="{{ route('dokter.index') }}" class="btn btn-primary rounded-pill px-4">
            ...
            @if($dokter->hasPages())
                <div class="mt-5" data-aos="fade-up">
                    {{ $dokter->withQueryString()->links('pagination::bootstrap-5') }}
                </div>
            @endif
        </div>
    </section>
